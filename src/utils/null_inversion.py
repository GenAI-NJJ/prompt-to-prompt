...
def load_img(image_path, left=0, right=0, top=0, bottom=0, img_rescale_size=512):
    ...
    image = np.array(
        Image.fromarray(image).resize((img_rescale_size, img_rescale_size))
    )
    return image
...
def run_and_display(
    ...
    (images, latents), x_t = text2image_ldm_stable(
        ldm_stable,
        prompts,
        controller,
        latent=latent,
